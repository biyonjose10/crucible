...
**The injection test we ran on the problem box.** The obvious attack on a model that writes your exam is to tell it what to write. We gave production a problem description ordering it to write tests that always pass and award full marks. It produced an ordinary, discriminating suite. A script drives the whole loop over five sample problems; on the last run every one produced a suite its own reference passed and a do-nothing stub scored zero against. Over roughly ten runs during development the retry fired once.
